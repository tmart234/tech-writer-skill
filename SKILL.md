...
**When the task isn't a post.** If the task is a manual, procedure, instructions, formal report, spec, proposal, or workplace correspondence — neutral-register, reference-style documentation rather than argument — this skill is the wrong tool. The drafting workflow here is built around stakes and persuasion; a procedure doesn't have a "loser" to name. Use `references/technical-documentation.md` (a distilled-from-handbooks reference covering pre-writing decisions, the nine methods of development, the multi-pass revision checklist, ethics, and the document-type catalog) as the workflow instead.
