...
- `toolkit.md`: the influences (writers to steal moves from, per post) and the register theory (Measured / Sharp / tactical landings). Loaded in Pass 0.
- `beat-notes.md`: beat-specific craft. Evidence types, stakes patterns, anti-patterns, and worked examples that only apply to this beat. Loaded in Pass 0, Pass 0.5, and the Pass 2 anti-pattern scan.
...
`SKILL.md` has one hook, the "Voice profile" section near the top, that checks whether this folder exists. When it does, the five-pass workflow loads these files at the points marked in `SKILL.md` and `references/multi-pass.md`. When it doesn't, every one of those load steps is skipped and the core runs as a standalone generic technical-writing skill.
